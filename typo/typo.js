 ...
 * Typo constructor.
 *
 * @param {String} [language] The locale code of the dictionary being used. e.g.,
...
var Typo = function (language, affData, wordsData, settings){
	settings = settings || {};
	...
	if(language){
		...
		if('chrome' in window && 'extension' in window.chrome && 'getURL' in window.chrome.extension){
			if(!affData)
				affData = this._readFile(chrome.extension.getURL("lib/typo/dictionaries/" + language + "/" + language + ".aff"));
			if(!wordsData)
				wordsData = this._readFile(chrome.extension.getURL("lib/typo/dictionaries/" + language + "/" + language + ".dic"));
		}
		else{
			var path = settings.dictionaryPath || '';
			if(!affData)
				affData = this._readFile(path + "/" + language + "/" + language + ".aff");
			if(!wordsData)
				wordsData = this._readFile(path + "/" + language + "/" + language + ".dic");
		}
		...
		//if we add this ONLYINCOMPOUND flag to this.compoundRuleCodes, then _parseDIC will do the work of saving the list of words that are compound-only
